...
class SlackClient:
    ...
    async def get_channel_name(self, channel_id: str) -> str | None:
        """Resolve a readable conversation name from a Slack channel id."""
        try:
            ...
            # DM channels do not expose a channel name, so use the peer user's name instead.
    ...
    async def get_sender_name(self, user_id: str) -> str | None:
        """Resolve a readable Slack user name from a user id."""
        try:
